...
const listVersions = asyncHandler(async (req, res) => {
  ...
  if (req.query.userId) {
    filter.userId = await resolveUserId(req.query.userId);
  }
...
const getVersion = asyncHandler(async (req, res) => {
  ...
  if (version.userId.toString() !== (await resolveUserId(req.user?.userId))?.toString() && process.env.NODE_ENV === "production") {
...
const createVersion = asyncHandler(async (req, res) => {
  const { resumeId, userId: rawUserId, resumeJSON, sourceType = "blank", uploadedFile } = req.body;

  if (!resumeId || !rawUserId || !resumeJSON) {
...
const updateVersion = asyncHandler(async (req, res) => {
  ...
  if (req.body.resumeJSON) {
    version.resumeJSON = req.body.resumeJSON;
    version.markModified("resumeJSON");
  }
  ...
  // Handle other top-level fields if any
  const otherFields = ["sourceType", "uploadedFile", "versionNumber"];
  otherFields.forEach(field => {
    if (req.body[field] !== undefined) {
      version[field] = req.body[field];
      version.markModified(field);
    }
  });
