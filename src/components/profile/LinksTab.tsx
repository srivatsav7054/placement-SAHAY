  ...
  useEffect(() => {
    const fetchLinks = async () => {
      try {
        ...
        // Get user from DB using clerk ID
        // apiRequest already unwraps data, so response is the user object
        const response = await apiRequest(`/users/by-clerk-id/${clerkUser.id}`, {
          method: "GET",
        });
