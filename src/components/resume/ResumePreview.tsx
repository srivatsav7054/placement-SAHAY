...
const ResumePreview = ({ resume, clerkUserId }: ResumePreviewProps) => {
  ...
  const fullName = profileData.fullName || `${userProfile.firstName} ${userProfile.lastName}`;
  const education = resume.resumeJSON?.education || [];
  const skills = resume.resumeJSON?.skills || [];
  const experience = resume.resumeJSON?.experience || [];
  const projects = resume.resumeJSON?.projects || [];
  const certificates = resume.resumeJSON?.certificates || [];
        ...
        return (
          <div key={id}>
            ...
            <p className={cn("text-muted-foreground", isCompact ? "text-[11px] leading-relaxed" : "text-xs leading-relaxed")}>
              {resume.summary || profileData.summary || "Professional summary goes here."}
            </p>
          </div>
        );
      case "education":
          ...
          <div key={id}>
            ...
            {education.map((e: any, i: number) => (
              <div key={i} className="flex justify-between mb-2">
                ...
                <p className="text-[11px] text-muted-foreground shrink-0">{e.startDate} - {e.endDate}</p>
              </div>
            ))}
          </div>
        ) : null;
