...
const ATSScoreTab = () => {
  ...
  const handleAnalyze = () => {
    ...
    // Simulate a scan but use the actual data from the selected version
    const selected = (data || []).find(r => r.latestVersionId?._id === selectedVersionId);
    const score = selected?.latestVersionId?.metadata?.jobMatchScore || 72; // Fallback to 72 if no analysis run yet
    ...
    setTimeout(() => {
      setResults({
        ...
        breakdown: [
          ...
          { label: "Keywords", score: score - 10, status: score > 75 ? "good" : "medium" as const },
          { label: "Structure", score: 85, status: "good" as const },
        ...
        suggestions: score < 80 ? [
          ...
          "Keep refreshing your keywords every 3 months."
        ],
      });
      setAnalyzing(false);
    }, 1200);
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-card border-border">
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleAnalyze} disabled={analyzing || !selectedVersionId} className="w-full sm:w-auto">
