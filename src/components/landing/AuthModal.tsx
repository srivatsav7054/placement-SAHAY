...
/* ─── Clerk-backed auth ─────────────────────────────────────────────────── */
const ClerkAuthContent = ({ mode, onClose }: AuthModalProps) => {
  const { isSignedIn } = useAuth();

  useEffect(() => {
    if (mode && isSignedIn) onClose();
  }, [isSignedIn, mode, onClose]);
  ...
  const sharedAppearance = {
    elements: {
      ...
      card:
        "w-full rounded-[1.75rem] border border-white/50 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(247,250,251,0.96))] px-6 py-8 shadow-[0_30px_80px_rgba(15,23,42,0.22)] sm:px-8 sm:py-8",
      header: "space-y-2 pb-6",
      headerTitle:
        "mx-auto max-w-[320px] text-center font-heading text-[1.75rem] font-bold leading-tight text-foreground sm:text-[2rem]",
      headerSubtitle:
        "mx-auto max-w-[320px] text-center text-sm leading-6 text-muted-foreground",
      socialButtonsBlockButton:
        "flex h-12 w-full items-center justify-center gap-3 rounded-2xl border border-border/70 bg-card/80 px-4 text-sm font-medium shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-secondary",
      socialButtonsBlockButtonText: "text-sm font-medium text-foreground",
      formButtonPrimary:
        "mt-2 flex h-12 w-full items-center justify-center rounded-2xl bg-primary px-4 text-sm font-semibold text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary/90 active:scale-[0.99]",
      formFieldInput:
        "h-12 rounded-2xl border border-border/70 bg-background px-4 text-sm text-foreground shadow-sm transition-all focus:border-primary focus:ring-2 focus:ring-primary/40",
      formFieldLabel: "mb-2 block text-sm font-semibold text-foreground",
      formField: "space-y-2",
      formFieldRow: "mb-5",
      dividerRow: "my-6",
      dividerLine: "bg-border/50",
      dividerText:
        "px-3 text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground/80",
      footer: "pt-5",
      footerAction: "text-center text-sm text-muted-foreground",
      footerActionLink: "font-bold text-primary transition-colors hover:text-primary/80",
      identityPreviewText: "font-medium text-foreground",
      identityPreviewEditButton:
        "rounded-md px-2 py-1 text-primary transition-colors hover:bg-primary/10",
      form: "space-y-1",
      main: "space-y-1",
      formResendCodeLink: "font-semibold text-primary hover:text-primary/80",
      otpCodeFieldInput: "h-12 w-12 rounded-2xl border border-border/70",
      alert: "rounded-xl border border-orange-200/80 bg-orange-50 px-4 py-3 text-sm text-orange-900",
...
/* ─── Fallback form (no Clerk) ───────────────────────────────────────────── */
const FallbackAuthForm = ({
  mode,
  onToggle,
}: {
  mode: "signIn" | "signUp";
        ...
        >
          ...
          >
            ...
            <div className="relative flex justify-center">
              {CLERK_ENABLED ? (
                <ClerkAuthContent mode={localMode} onClose={onClose} />
              ) : (
                <FallbackAuthForm
                  mode={localMode}
                  onToggle={() =>
