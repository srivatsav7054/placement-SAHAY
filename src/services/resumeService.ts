...
export const listResumes = async (clerkUserId: string) => {
  try {
    ...
    const userResponse = await apiRequest<any>(`/users/by-clerk-id/${clerkUserId}`, {
      method: "GET",
    });

    // Then get resumes for that user
    return await apiRequest<ResumeRecord[]>(`/resumes?userId=${userResponse._id}`);
  } catch (error) {
